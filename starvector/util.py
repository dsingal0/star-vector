...
def get_exp_id(config):
    ...
    if config.model.model_name is not None:
        ...
        model_name = (
            config.model.starcoder_model_name.split("/")[1]
            + "_"
            + config.model.image_encoder_type
        )
    exp_id = (
        f"{config.project.project}-{config.model.max_length}-{model_name}-{exp_hash_id}"
    )
...
def subprocess_call(cmd_string):
    ...
    return subprocess.check_output(
        shlex.split(cmd_string), shell=False, stderr=subprocess.STDOUT
    ).decode("utf-8")
...
def copy_code(
    ...
    rsync_avialable = (
        len(subprocess.run(["which", "rsync"], capture_output=True, text=True).stdout)
        > 0
    )

    if rsync_avialable:  # TODO: validate this works
        rsync_cmd_base = (
            "rsync -av -r -q --delete-before --exclude='.*' --exclude '__pycache__/'"
        )

        exclude_options = " ".join(
            [f"--exclude='{filename}'" for filename in exclude_list]
        )
        ...
        for dirpath, dirs, files in os.walk(src_path):
            ...
            for filename in fnmatch.filter(files, "*"):
                ...
                if not os.path.isfile(
                    dst_file
                ):  # check if destination is already a file
...
def get_config():
    ...
    specific_conf = (
        OmegaConf.load(cli_conf.pop("config")) if "config" in cli_conf else {}
    )
    config = OmegaConf.merge(base_conf, specific_conf, cli_conf)
    if config.training.resume_from_checkpoint:
        if not os.path.exists(
            os.path.join(
                os.path.dirname(config.training.resume_from_checkpoint), "config.yaml"
            )
        ):
            config.training.resume_from_checkpoint = get_last_checkpoint(
                config.training.resume_from_checkpoint
            )
            cli_conf.training.resume_from_checkpoint = (
                config.training.resume_from_checkpoint
            )
        pretrained_conf = OmegaConf.load(
            os.path.join(
                os.path.dirname(config.training.resume_from_checkpoint), "config.yaml"
            )
        )

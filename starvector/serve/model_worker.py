...
class ModelWorker:
    ...
    def generate_stream(self, params):
        tokenizer, model, image_processor, task = (
            self.tokenizer,
            self.model,
        ...
        streamer = TextIteratorStreamer(
            tokenizer, skip_prompt=False, skip_special_tokens=True, timeout=15
        )
        ...
        if task == "Image2SVG":
            ...
            # White PIL image
            batch["image"] = torch.zeros((3, 256, 256), dtype=torch.float16).to(
                self.model.device
            )

...
class ImageEncoder(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        if self.image_encoder_type == "clip":
            self.visual_encoder, self.ln_vision = self.build_clip_encoder(
    ...
    def build_clip_encoder(self, image_size):
        from starvector.model.image_encoder.clip_model import (
            VisionTransformer,
            LayerNorm,
        )
    ...
    def build_vqgan_encoder(self):
        ...
        vqgan_config_file = [
            file for file in files_in_directory if file.endswith("project.yaml")
        ][0]
        vqgan_config = OmegaConf.load(
            os.path.join(vqgan_chkp_path, "configs", vqgan_config_file)
        )
        visual_encoder = Encoder(**vqgan_config.model.params.ddconfig)

        # Load checkpoint weights
        checkpoint = torch.load(
            os.path.join(vqgan_chkp_path, "checkpoints", "last.ckpt")
        )["state_dict"]
    ...
    def build_convnext_encoder(self):
        ...
        model, _, _ = open_clip.create_model_and_transforms(
            "convnext_base_w", pretrained="laion2b_s13b_b82k"
        )
        return model.visual

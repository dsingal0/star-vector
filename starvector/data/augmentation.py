import numpy as np
from svgpathtools import Path, Arc, CubicBezier, QuadraticBezier, svgstr2paths
import os
...
class SVGTransforms:
    def __init__(self, transformations):
        ...
        self.colors = self.transformations.get(
    ...
    def sample_transformations(self):
        ...
        if self.shift_re or self.shift_im:
            self.shift_real = np.random.uniform(
                self.shift_re["from"], self.shift_re["to"]
            )
            self.shift_imag = np.random.uniform(
                self.shift_im["from"], self.shift_im["to"]
            )
        ...
        if self.color_noise:
            self.color_noise_std = np.random.uniform(
                self.color_noise["from"], self.color_noise["to"]
            )

    def paths2str(
        self,
        groupped_paths,
        svg_opening_tag='<svg xmlns="http://www.w3.org/2000/svg" version="1.1">',
    ):
        keys_to_exclude = ["d", "cx", "cy", "rx", "ry"]
        all_groups_srt = ""
        for group, elements in groupped_paths.items():
            group_attributes, paths_and_attributes = (
                elements.get("attrs", {}),
                elements.get("paths", []),
            )
            group_attr_str = " ".join(
                f'{key}="{value}"' for key, value in group_attributes.items()
            )
            path_strings = []
            path_str = ""
    ...
    def add_noise(self, seg):
        ...
        if self.noise_type == "gaussian":
            ...
            noise_sample = (
                complex(
                    pnoise1(np.random.random(), octaves=2),
                    pnoise1(np.random.random(), octaves=2),
                )
                * noise_scale
            )
    ...
    def do_rotate(self, path, viewbox_width, viewbox_height):
        if self.rotate:
            new_path = path.rotated(
                self.rotation_angle, complex(viewbox_width / 2, viewbox_height / 2)
            )
            return new_path
        else:
            return path
    ...
    def add_color_noise(self, source_color):
        ...
        if source_color.startswith("#"):
            ...
            base_color = mcolors.hex2color(
                mcolors.CSS4_COLORS.get(source_color, "#FFFFFF")
            )

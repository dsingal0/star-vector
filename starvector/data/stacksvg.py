...
class SVGStackDataset(SVGDatasetBase):
    def __init__(self, dataset_name, split, im_size, num_samples=-1, **kwargs):
        ...
        self.color_changer = SVGTransforms(
            {
                "color_change": True,
        ...
        if select_dataset_name:
            self.data = self.data.filter(
                lambda example: example["model_name"] == select_dataset_name
            )
        ...
        if self.image_processor and "siglip" in self.image_processor:
            model_name = {
                "siglip_512": "google/siglip-base-patch16-512",
                "siglip_384": "google/siglip-large-patch16-384",
                "siglip_256": "google/siglip-base-patch16-256",
    ...
    def __getitem__(self, idx):
        ...
        caption = random.choice(text2svg_captions) + self.data[idx].get(
            caption_column, ""
        )
        return {
            "svg": svg,

...
class ImageTrainProcessor(ImageBaseProcessor):
    def __init__(self, mean=None, std=None, size=224, **kwargs):
        ...
        self.transform = transforms.Compose(
            [
                transforms.Lambda(
                    lambda img: self._rgba_to_rgb_white(img)
                    if img.mode == "RGBA"
                    else img
                ),
...
def process_and_rasterize_svg(svg_string, resolution=256, dpi=128, scale=2):
    try:
        svgstr2paths(
            svg_string
        )  # This will raise an exception if the svg is still not valid
        out_svg = svg_string
    except:
        try:
...
def find_unclosed_tags(svg_content):
    ...
    for tag in all_tags:
        if all_tags.count(tag) > self_closing_tags.count(tag) + svg_content.count(
            "</" + tag + ">"
        ):
...
# -------------- Plotting utils --------------
def plot_images_side_by_side_with_metrics(
    image1, image2, l2_dist, CD, post_processed, out_path
):
    ...
    axes[2].axis("off")
    plt.suptitle(
        f"MSE: {l2_dist:.4f}, CD: {CD:.4f}, post-processed: {str(post_processed)}",
        fontsize=16,
        y=1.05,
    )
...
def plot_images_side_by_side_temperatures(
    samples_temp, metrics, sample_dir, outpath_filename
):
    # Create a plot with the original image and different temperature results
    num_temps = len(samples_temp)
    fig, axes = plt.subplots(
        2, num_temps + 1, figsize=(15, 4), gridspec_kw={"height_ratios": [10, 2]}
    )

    # Plot the original image
    gt_image_path = os.path.join(
        sample_dir, f"temp_{list(samples_temp.keys())[0]}", f"{outpath_filename}_or.png"
    )
    ...
    for idx, (temp, sample) in enumerate(samples_temp.items()):
        gen_image_path = os.path.join(
            sample_dir, f"temp_{temp}", f"{outpath_filename}.png"
        )
...
def plot_images_and_prompt_with_metrics(
    prompt, svg_raster, gt_svg_raster, clip_score, post_processed, out_path
):
    # First col shows caption, second col shows generated svg, third col shows gt svg
    fig, axes = plt.subplots(1, 3, figsize=(10, 5))
    ...
    axes[2].axis("off")
    plt.suptitle(
        f"CLIP Score: {clip_score:.4f}, post-processed: {str(post_processed)}",
        fontsize=16,
        y=1.05,
    )
    plt.savefig(out_path, bbox_inches="tight", pad_inches=0.1)
...
def plot_images_and_prompt_temperatures(
    prompt, samples_temp, metrics, sample_dir, outpath_filename
):

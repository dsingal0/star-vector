...
class SVGValidator(ABC):
    def __init__(self, config):
        ...
        if config.model.from_checkpoint:
            ...
            self.out_dir = (
                ...
                + config.run.out_dir
                + "/"
                + config.model.generation_engine
                + "_"
    ...
    def create_comparison_plot(
        self, sample_id, gt_raster, gen_raster, metrics, output_path
    ):
        """
        ...
        fig, (ax_metrics, ax_images) = plt.subplots(
            2, 1, figsize=(12, 8), gridspec_kw={"height_ratios": [1, 4]}
        )
    ...
    def create_comparison_plots_with_metrics(self, all_metrics):
        ...
        for sample_id, metrics in all_metrics.items():
            ...
            comp_img = self.create_comparison_plot(
                sample_id, gt_raster, gen_raster, metrics, output_path
            )
            ...
            if (
                ...
                and sample_id in self.table_data
                and self.config.run.log_images
            ):
                import wandb
    ...
    def _save_svg_files(self, sample_dir, outpath_filename, res):
        ...
        # Save SVG files
        with open(
            os.path.join(sample_dir, f"{outpath_filename}.svg"), "w", encoding="utf-8"
        ) as f:
            f.write(res["svg"])
        with open(
            os.path.join(sample_dir, f"{outpath_filename}_raw.svg"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(res["svg_raw"])
        with open(
            os.path.join(sample_dir, f"{outpath_filename}_gt.svg"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(res["gt_svg"])
    ...
    def validate(self):
        ...
        for i, batch in enumerate(tqdm(self.dataloader, desc="Validating")):
            if self.config.generation_params.generation_sweep:
                ...
                results = self.generate_and_process_batch(
                    batch, self.config.generation_params
                )
    ...
    def preprocess_results(self):
        ...
        batch = {"gen_svg": [], "gt_svg": [], "gen_im": [], "gt_im": [], "json": []}
    ...
    def generate_and_process_batch(self, batch, generate_config):
        ...
        processed_results = [
            self.post_process_svg(output) for output in generated_outputs
        ]
    ...
    def get_validator(cls, key, args, validator_configs):
        ...
        if key not in validator_configs:
            ...
            raise ValueError(
                f"Validator '{key}' is not recognized. Available validators: {available_validators}"
            )

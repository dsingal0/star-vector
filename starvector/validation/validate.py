...
def get_validator(validator_name, config):
    ...
    if config.model.generation_engine.lower() in ENGINE_MAPPING:
        config.model.generation_engine = ENGINE_MAPPING[
            config.model.generation_engine.lower()
        ]
...
if __name__ == "__main__":
    ...
    if "config" not in cli_conf:
        raise ValueError(
            "No config file provided. Please provide a config file using 'config=path/to/config.yaml'"
        )

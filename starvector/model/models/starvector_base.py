...
class StarVectorBase(nn.Module, ABC):
    def __init__(self, config, **kwargs):
        ...
        if self.use_image_encoder():
            ...
            # Build Adapter
            self.image_projection = self.get_adapter(config, **kwargs).to("cuda")
        else:
            self.query_length = 0

        self.max_length = (
            config.max_length_train - self.query_length - 4
        ...
        # Freeze parameters
        self.freze_parameters(
            self.train_image_encoder, self.train_LLM, self.train_connector
        )
    ...
    def get_adapter(self, config, **kwargs):
        ...
        vision_hidden_size, self.query_length = self.get_hidden_size_and_query_length(
    ...
    def embed_im_to_svg(self, batch, device):
        ...
        embedded_image = self.image_encoder(image)
        conditioning_embeds = self.image_projection(embedded_image)
        conditioning_embeds_att = torch.ones(
            conditioning_embeds.size()[:-1], dtype=torch.long
        ).to(device)
        ...
        empty_targets = (
            torch.ones(conditioning_embeds_att.size(), dtype=torch.long)
            .to(device)
            .fill_(-100)
        )
        ...
        attention_mask = torch.cat(
            [conditioning_embeds_att, svg_tokens.attention_mask], dim=1
        )
    ...
    def forward(self, batch):
        ...
        # Depending
        if task == "text2svg":
            inputs_embeds, attention_mask, targets = self.embed_text_to_svg(
                batch, device
            )
        elif task == "im2svg":
    ...
    def _prepare_generation_inputs(self, batch, prompt, device):
        ...
        embedded_att = torch.ones(embedded_image.size()[:-1], dtype=torch.long).to(
            "cuda"
        )
    ...
    def _get_generation_kwargs(self, base_kwargs):
        ...
        end_sequence = self.svg_transformer.tokenizer(
            "</svg>", add_special_tokens=False
        )["input_ids"]
        stopping_criteria = StoppingCriteriaList(
            [StoppingCriteriaSub(stops=[end_sequence])]
        )
        return {
    ...
    def generate_im2svg(self, batch, **kwargs):
        ...
        raw_svg = self.svg_transformer.tokenizer.batch_decode(
    ...
    def generate_im2svg_grpo(self, batch, **kwargs):
        ...
        outputs = self.svg_transformer.transformer.generate(**generation_kwargs)
        outputs = torch.cat(
            [prompt_tokens.input_ids.repeat(num_return_sequences, 1), outputs], dim=1
        )
        raw_svg = self.svg_transformer.tokenizer.batch_decode(
            outputs, skip_special_tokens=True
        )
    ...
    def generate_text2svg(self, batch, **kwargs):
        ...
        input_tokens = torch.cat(
            [prompt_tokens.input_ids, trigger_token.input_ids], dim=1
        )
        attention_mask = torch.cat(
            [prompt_tokens.attention_mask, trigger_token.attention_mask], dim=1
        )

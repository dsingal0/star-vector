...
class LPIPSDistanceCalculator(BaseMetric):
    ...
    def LPIPS(self, tensor_image1, tensor_image2):
        tensor_image1, tensor_image2 = (
            tensor_image1.to(self.device),
            tensor_image2.to(self.device),
        )
    ...
    def calculate_score(self, batch, batch_size=8, update=True):
        ...
        for tensor_gt_batch, tensor_gen_batch in tqdm(data_loader):
            ...
            values.extend(
                [lpips_values.squeeze().cpu().detach().tolist()]
                if lpips_values.numel() == 1
                else lpips_values.squeeze().cpu().detach().tolist()
            )
